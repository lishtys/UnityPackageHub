    ...
    public class RuntimeDebugger : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            if (AllowDebugging)
            {
                if (_expansion)
                {
                    _windowRect = GUI.Window(0, _windowRect, ExpansionGUIWindow, "iForrest - DEBUGGER");
                }
                else
                {
                    _windowRect = GUI.Window(0, _windowRect, ShrinkGUIWindow, "iForrest - DEBUGGER");
                }
            }
        }
        ...
        private void ExpansionGUIWindow(int windowId)
        {
            ...
            GUI.contentColor = (_debugType == DebugType.GameManagerTool ? Color.white : Color.gray);
            if (GUILayout.Button("Game Manager Tool", GUILayout.Height(30)))
            {
                _debugType = DebugType.GameManagerTool;
            }
            ...
            else if (_debugType == DebugType.GameManagerTool)
            {
                GUILayout.BeginVertical("Box");
               
                GUILayout.Label($" add game manager logic here");
